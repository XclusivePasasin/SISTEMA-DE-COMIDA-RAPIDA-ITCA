...
{
    class MesaDAO : ConnectionDataBase
    {
        ...
        public bool Insertar(object objDatos)
        {
            ...
            m = (ClsMesa)objDatos;
            string sql = "INSERT INTO Mesa VALUES('"+m.NumeroMesa+"', "+m.Estado+"); ";
            if (Ejecutar(sql))
            {
                return true;
            }
        ...
        public bool Modificar(object objDatos)
        {
            ClsMesa m = new ClsMesa();
            m = (ClsMesa)objDatos;
            string sql = "UPDATE Mesa SET Estado = "+m.Estado+", Numero_Mesa = '"+m.NumeroMesa+"' WHERE idMesa =" + m.IdMesa;
            if (Ejecutar(sql))
            {
                return true;
            }
        ...
        {
            string sql = "DELETE FROM Mesa WHERE idMesa=" + CodigoUsuario;
            if (Ejecutar(sql))
            {
                return true;
            }
        ...
        public DataTable Buscar(string Campo, string ValorCampo)
        {
            ...
            if (Campo == "Codigo")
            {
                sql = "SELECT * FROM V_Mesa WHERE CODIGO=" + ValorCampo;
            }
            else
            {
                sql = "SELECT * FROM V_Mesa WHERE " + Campo + " Like '%" + ValorCampo + "%'";
            }
